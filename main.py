...
class WindowLoop(Functionalities, Reports_pdf):
    def __init__(self):
    ...
    def screen(self):
        ...
        self.window.title(f"Consuela - Welcome {self.PC_NAME}")
        self.window.configure(background=hex_colors["color_3"])
        self.window.geometry(f"{custom_size['init_width']}x{custom_size['init_height']}")
        self.window.resizable(True, True)
        self.window.maxsize(width=get_user_width, height=get_user_height)
        self.window.minsize(width=custom_size['min_width'], height=custom_size['min_height'])
        self.window.iconbitmap('img/favicon.ico')
        self.img = ImageTk.PhotoImage(Image.open("img/consu_logo_minor.png"))

    def screen_frames(self):
        self.frame_1 = Frame(self.window)
        self.frame_1.place(relx=0.01, rely=0.01, relwidth=0.3, relheight=0.97)
        self.frame_1.configure(background=hex_colors["color_1"])
        # ====================================================
        self.frame_2 = Frame(self.window, highlightbackground=hex_colors["borda"], highlightthickness=1)
        self.frame_2.place(relx=0.350, rely=0.050, relwidth=0.63, relheight=0.850)
        self.frame_2.configure(background=hex_colors["color_1"])

    def widgets_frame_1(self):
        self.canvas_btn = Canvas(self.frame_1, bd=0, bg='black', highlightbackground='gray', highlightthickness=3)
        self.canvas_btn.place(relx=0.050, rely=0.800, relwidth=0.8, relheight=0.165)

        # =========== BUTTON ================
        # entrar
        self.btn_enter = Button(self.frame_1, text="Salvar", bd=2, bg=hex_colors["color_2"], font=('consolas', 12, 'bold'),
        ...
        self.label_credits = Label(self.frame_1, text="Por Caio Madeira:", bg=hex_colors["color_1"],
                                   font=('helvetica', 7, 'bold'))
        self.label_credits.place(x=10, y=5, width=100, height=20)

        self.label_account = Label(self.frame_1, text="Conta:", bg=hex_colors["color_1"], font=('consolas', 15, 'bold'))
        self.label_account.place(relx=0.01, rely=0.40, relwidth=0.45, relheight=0.05)

        self.label_user = Label(self.frame_1, text="Usuário/E-mail:", bg=hex_colors["color_1"],
                                font=('consolas', 12, 'bold'))
        self.label_user.place(relx=0.10, rely=0.50, relwidth=0.45, relheight=0.05)

        self.label_pass = Label(self.frame_1, text="Senha:", bg=hex_colors["color_1"], font=('consolas', 15, 'bold'))
    ...
    def Menus(self):
        ...
        menu_bar.add_cascade(label="Mais", menu=menu_file_limpar)
